document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadTeamPosts() {
    try {
      ...
      for (const post of posts) {
        ...
        if (isOwner) {
          const reqResponse = await fetch(`http://localhost:5000/api/join-requests/post/${post._id}`);
      ...
      document.querySelectorAll(".request-btn").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          try {
            const response = await fetch("http://localhost:5000/api/join-requests", {
              method: "POST",
      ...
      document.querySelectorAll(".accept-btn").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          const id = e.target.dataset.id;
          try {
            await fetch(`http://localhost:5000/api/join-requests/${id}`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
  ...
  async function loadMyRequests() {
    try {
      const response = await fetch(`http://localhost:5000/api/join-requests/user/${user._id}`);
