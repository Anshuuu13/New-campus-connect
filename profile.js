document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const skillsResponse = await fetch(`http://localhost:5000/api/skills/${profileUserId}`);
  ...
  // Load achievements
  try {
    const achResponse = await fetch(`http://localhost:5000/api/achievements/user/${profileUserId}`);
  ...
  try {
    const projResponse = await fetch(`http://localhost:5000/api/projects/public/${profileUserId}`);
  ...
  try {
    const userResponse = await fetch(`http://localhost:5000/api/users/${profileUserId}`);
